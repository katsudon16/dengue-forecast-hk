# Modified MASS:stepAIC to accommodate AICc
# stepAICc_any = a function that accommodates AICc/AIC and glm/glmm
# for simpler use:
# - for models similar to glm, use stepAICc
# - for glmmTMB, use stepAICc_MM
stepAICc_any <- function (object, scope, scale=0, skipTerm=F,
                     direction=c("both", "backward", "forward"),
                     trace=1, keep=NULL, steps=1000, use.start=FALSE,
                     k=2, useAICc=FALSE, ...) {
  library("MuMIn")
  selectorString <- ifelse(useAICc == TRUE, "AICc", "AIC")
  ...
  step.results <- function(models, fit, object, usingCp = FALSE) {
    ...
    AIC <- sapply(models, "[[", selectorString)
    ...
    aod <- if (usingCp)
      ...
      data.frame(Step = change, Df = ddf, Deviance = dd,
                    `Resid. Df` = rdf, `Resid. Dev` = rd, AICc = AIC,
                    check.names = FALSE)
    else
      data.frame(Step = change, Df = ddf, Deviance = dd,
  ...
  edf <- bAIC[1L]
  bAIC <- ifelse(useAICc == TRUE, AICc(fit, k=k), bAIC[2L])
  if (is.na(bAIC))
    stop("AIC/AICc is not defined for this model, so 'stepAIC' cannot proceed")
  if (bAIC == -Inf)
    stop("AIC/AICc is -infinity for this model, so 'stepAIC' cannot proceed")
  nm <- 1
  Terms <- terms(fit)
  if (trace) {
    cat("Start:  ", selectorString, "=", format(round(bAIC, 2)), "\n", cut.string(deparse(formula(fit))),
        "\n\n", sep = "")
    utils::flush.console()
  }
  if (useAICc)
  ...
  while (steps > 0) {
    ...
    bAIC <- ifelse(useAICc == TRUE, AICc(fit, k=k), bAIC[2L])
    if (trace) {
      cat("\nStep:  ", selectorString, "=", format(round(bAIC, 2)), "\n",
          cut.string(deparse(formula(fit))), "\n\n", sep = "")
      utils::flush.console()
    }
    if (bAIC >= AIC + 1e-07)
