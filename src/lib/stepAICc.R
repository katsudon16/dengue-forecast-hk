stepAICc <-
  function (object, scope, scale = 0, AICc=T, direction = c("both", "backward", 
            ...
            k = 2, ...) 
  {
    ...
    step.results <- function(models, fit, object, usingCp = FALSE) {
      ...
      bAIC <- sapply(models, "[[", "bAIC")
      heading <- c("Stepwise Model Path \nAnalysis of Deviance Table", 
                   "\nInitial Model:", deparse(formula(object)), "\nFinal Model:", 
                   deparse(formula(fit)), "\n")
      aod <- if (usingCp) { 
        data.frame(Step = change, Df = ddf, Deviance = dd, 
                   `Resid. Df` = rdf, `Resid. Dev` = rd, Cp = bAIC, 
                   check.names = FALSE) 
          }
      else {
        data.frame(Step = change, Df = ddf, Deviance = dd, 
                      `Resid. Df` = rdf, `Resid. Dev` = rd, bAIC = bAIC, 
                      check.names = FALSE)
      }
    ...
    bAIC <- bAIC + (2*(length(fit$coeff))*(length(fit$coeff)+1))/(length(object$res)-length(fit$coeff)-1)
    if (is.na(bAIC)) 
    ...
    while (steps > 0) {
      ...
      bAIC <- bAIC[2L]
      #print(bAIC)
      print(length(fit$coeff))
      print(length(object$res))
      bAIC <- bAIC + (2*(length(fit$coeff))*(length(fit$coeff)+1))/(length(object$res)-length(fit$coeff)-1)
      print(bAIC)
      #stop(bAIC)
      if (trace) {
